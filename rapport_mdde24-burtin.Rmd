...
Pour le premier traitement, il fut repris la dose retenue par l'article à savoir 10 mg/kg et par jour, avec une durée de perfusion de 30min. La dose croit selon le poids, et l'intercepte demandé sur Simulix est de 0. Au départ un modèle sur 7 jour a été réalisé, mais pour plus de simplicité sur le rapport (et comme il n'est pas fait mention de durée totale de traitement dans l'article), on se basera sur une dose unique.

Le model, dénommé "10mgkg_uniqueDose_withCov.slmx" est disponible dans "models/Simulix Project/".

La comparaison se faite avec un traitement unique pour tous à 150 mg, avec donc absence de scaling selon le poids. Toujours en dose unique, et avec un délai de perfusion de 30min.

# Résultats

```{r importation des données, include=FALSE}
parametres_ind = read.csv(file = "models/Simulix Project/10mgkg_7day_withCov/10mgkg_uniqueDose_withCov/Simulation/individualParameters.txt")
```

```{r prétraitement_parametres}
# On peut renommer les variables si nécessaire
parametres_ind = parametres_ind %>% mutate("group" = case_when(parametres_ind$group == "simulationGroup1" ~ "10mg", parametres_ind$group == "simulationGroup2"~ "10mgkg", TRUE ~ "error"))
...
La figure X décrit la valeur moyenne pour chacune des distributions de paramètres simulés. Comme nous le voyons, la CL est de 1.52 L/h, le volume de distribution de 1.86 L, l'age de 21 mois, et le poids de 8.8 kg.
...
Les distributions des valeurs de Cmax et Cmin selon les groupes sont présentées dans les figures 2 et 3 respectivement.
